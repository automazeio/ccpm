...
This command creates the initial project context documentation in `ccpm/context/` by analyzing the current project state and establishing comprehensive baseline documentation.
...
- Run: `ls -la ccpm/context/ 2>/dev/null`
- If directory exists and has files:
  - Count existing files: `ls -1 ccpm/context/*.md 2>/dev/null | wc -l`
...
- If `ccpm/` doesn't exist, create it: `mkdir -p ccpm/context/`
- Verify write permissions: `touch ccpm/context/.test && rm ccpm/context/.test`
...
**Common Issues:**
- **No write permissions:** "❌ Cannot write to ccpm/context/. Check permissions."
...
📁 Created context in: ccpm/context/
...
- Target directory: `ccpm/context/` (create if needed)
